...
import com.waither.config.AppleConfig;
import com.waither.model.Key;
import com.waither.model.Keys;
...
public class AppleUtils {
    ...
    public boolean verifyIdentityToken(String id_token) {
        ...
        try {
            ...
            if (!"20B20D-0S8-1K8".equals(payload.getClaim("nonce")) || !appleConfig.getISS().equals(payload.getIssuer()) || !appleConfig.getAUD().equals(payload.getAudience().get(0))) {
                return false;
            }
    ...
    private boolean verifyPublicKey(SignedJWT signedJWT) {
        ...
        try {
            String publicKeys = HttpClientUtils.doGet(appleConfig.getAPPLE_PUBLIC_KEYS_URL());
    ...
    public String createClientSecret() {
        ...
        JWSHeader header = new JWSHeader.Builder(JWSAlgorithm.ES256).keyID(appleConfig.getKEY_ID()).build();
        JWTClaimsSet claimsSet = new JWTClaimsSet();
        Date now = new Date();

        claimsSet.setIssuer(appleConfig.getTEAM_ID());
        claimsSet.setIssueTime(now);
        claimsSet.setExpirationTime(new Date(now.getTime() + 3600000));
        claimsSet.setAudience(appleConfig.getISS());
        claimsSet.setSubject(appleConfig.getAUD());
...
//        }
    ...
    private byte[] readPrivateKey() {
        ...
        Resource resource = new ClassPathResource(appleConfig.getKEY_PATH());
        byte[] content = null;
    ...
    public TokenResponse validateAuthorizationGrantCode(String client_secret, String code) {
        ...
        tokenRequest.put("client_id", appleConfig.getAUD());
        tokenRequest.put("client_secret", client_secret);
        tokenRequest.put("code", code);
        tokenRequest.put("grant_type", "authorization_code");
        tokenRequest.put("redirect_uri", appleConfig.getAPPLE_WEBSITE_URL());
    ...
    private TokenResponse getTokenResponse(Map<String, String> tokenRequest) {
        ...
        try {
            String response = HttpClientUtils.doPost(appleConfig.getAUTH_TOKEN_URL(), tokenRequest);
            ObjectMapper objectMapper = new ObjectMapper();
    ...
    public Map<String, String> getMetaInfo() {
        ...
        metaInfo.put("CLIENT_ID", appleConfig.getAUD());
        metaInfo.put("REDIRECT_URI", appleConfig.getAPPLE_WEBSITE_URL());
